...
function updateAuthUI() {
    ...
    // Ẩn/Hiện link đăng nhập
    $('.auth-login-link').toggle(!loggedIn);
    
    // Xử lý khối Dropdown đã đăng nhập
    if (loggedIn && user) {
        $('.auth-logged-in').removeClass('d-none').addClass('d-inline-block');
        const displayName = user.name && user.name.trim() !== "" ? user.name : user.account;
        $('.auth-user-name').text(displayName);
    } else {
        $('.auth-logged-in').addClass('d-none').removeClass('d-inline-block');
    }
}